...
**方式二：使用故事RAG系统（补充）**：
- **search_story_knowledge**：查询世界观设定、人物设定、修炼体系等核心信息
- **search_character_info**：查询特定人格的详细信息、性格特点、对话风格
- **search_plot_threads**：查询剧情线索、伏笔设计、支线规划等信息

### 阶段一：时间线优先检查（必须第一步执行）
**⚠️ 关键：时间线检查是最高优先级，必须在所有其他检查之前完成**

1. **建立人格时间线表**（从设定文档或story MCP获取）：
   - 使用search_story_knowledge查询"人格觉醒时间线"、"第二卷人格发展"等关键词
   - 使用search_character_info查询每个具体人格的觉醒时机
   - 标准时间线（需要验证确认）：
     - 第6章：小一(病娇)觉醒
     - 第17章：小二(吃货)觉醒  
     - 第23章：小七(数学家)觉醒
...
2. **逐一核验当前章节出现的每个人格**：
   - ✅ 该人格是否已在规定时间觉醒？
   - ❌ 如未觉醒但出现台词/行为 = 致命错误
   - ⚠️ 如是觉醒前兆，是否符合设定的前兆形式？

3. **零容忍原则**：
   - 任何人格提前出现都是**致命错误**，必须立即标记
   - 不允许"可能合理"的模糊判断，严格按时间线执行
...
### 🚨 致命错误类型（零容忍）
1. **人格时间线冲突**：任何人格提前于设定时间出现台词或行为
2. **世界观破坏**：修仙界人物使用现代概念或知识
3. **系统格式错误**：系统009未使用【】格式

### ⚠️ 重要错误类型
1. **人格特征不符**：人格言行与图鉴设定不一致
2. **术语转换错误**：现代词汇未正确转换为修仙术语
3. **称谓系统混乱**：师兄师姐等称呼使用错误
