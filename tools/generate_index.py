...
def generate_index_content(settings_data: Dict[str, List[Tuple[str, int, int]]]) -> str:
    ...
    # 0. 文件结构说明
    content_lines.append("索引结构：TOC部分显示文件→索引行号范围；各文件部分显示章节→原文件行号范围")
    ...
    for file_path in settings_data.keys():
        filename = Path(file_path).name
        content_lines.append(f"- {filename} → PLACEHOLDER:{file_path}")
    
    content_lines.append("")
    
    # 2. 每个文件的详细索引（每个文件都是一级标题）
    for file_path, sections in settings_data.items():
